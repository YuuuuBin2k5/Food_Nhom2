...
const SellerDashboard = () => {
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            const expiringSoon = products.filter(p => {
                ...
                const daysUntilExpiry = Math.ceil((new Date(p.expirationDate) - new Date()) / (1000 * 60 * 60 * 24));
    ...
    const getStatusBadge = (status) => {
        const config = {
            ACTIVE: { label: "Đang bán", className: "bg-green-100 text-green-700", icon: "✅" },
            PENDING_APPROVAL: { label: "Chờ duyệt", className: "bg-yellow-100 text-yellow-700", icon: "⏳" },
            PENDING: { label: "Chờ xác nhận", className: "bg-yellow-100 text-yellow-700", icon: "⏳" },
            CONFIRMED: { label: "Đã xác nhận", className: "bg-blue-100 text-blue-700", icon: "👨‍🍳" },
        };
        const item = config[status] || { label: status, className: "bg-gray-100 text-gray-700", icon: "●" };
        return (
            <span className={`px-2 py-1 rounded-full text-xs font-bold ${item.className}`}>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            ...
                            <p className="text-white/90 text-base mt-2">
                                Xin chào, <span className="font-bold">{user?.extraInfo?.shopName || user?.fullName}</span>! 👋
                            </p>
                        </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer"
                        ...
                        <p className="text-4xl font-bold text-[#0f172a]">{stats.totalProducts}</p>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer"
                        ...
                        <p className="text-[#334155] text-sm font-medium mb-1">Đơn chờ duyệt</p>
                        <p className="text-4xl font-bold text-[#0f172a]">{stats.pendingOrders}</p>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-lg transition-all hover:-translate-y-1">
                        ...
                        <p className="text-[#334155] text-sm font-medium mb-1">Sắp hết hạn</p>
                        <p className="text-4xl font-bold text-[#0f172a]">{stats.expiringSoon}</p>
                ...
                <div className="bg-gradient-to-r from-[#FF6B6B] via-[#FF8E53] to-[#FFC75F] rounded-2xl shadow-lg p-8 mb-8 text-white">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-5xl font-bold">{formatPrice(stats.totalRevenue)}</p>
                                    ...
                                    {recentProducts.map((product) => (
                                        ...
                                        >
                                            <div className="flex-1">
                                                <p className="font-semibold text-[#0f172a] text-sm">{product.name}</p>
                ...
                <div className="mt-8 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        >
                            ...
                            <p className="text-xs text-[#334155] mt-1">Đăng bán thực phẩm mới</p>
                        </button>
                        <button
                            onClick={() => navigate("/seller/orders")}
                        ...
                        >
                            ...
                            <p className="text-xs text-[#334155] mt-1">Quản lý đơn đặt hàng</p>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <p className="text-xs text-[#334155] mt-1">Cập nhật dữ liệu mới</p>
                        </button>
                    </div>
                </div>
            </div>
